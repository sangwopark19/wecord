...
- [ ] 소셜 로그인(Google/Apple OAuth) 및 글로벌 프로필 설정
- [ ] 첫 가입 시 스포티파이 스타일 크리에이터 선택 큐레이션
- [ ] 커뮤니티 탐색/검색 및 가입 (커뮤니티별 전용 닉네임)
- [ ] 이중 계정 구조 (글로벌 계정 + 커뮤니티별 프로필)
- [ ] 팬 피드 (텍스트/이미지/영상 게시, 무한스크롤)
- [ ] 크리에이터 피드 & 아티스트 멤버 시스템 (그룹/솔로)
- [ ] 댓글(대댓글 1depth), 좋아요, 크리에이터 답글 하이라이트
- [ ] 게시글/댓글 번역 버튼 + 앱 UI 5개 언어 i18n
- [ ] 알림 시스템 (푸시 알림, 커뮤니티/카테고리별 설정)
- [ ] 더보기 탭 (글로벌 프로필 편집, 앱 설정, 로그아웃)
- [ ] 홈 피드 (가입 0개: 추천 / 가입 1+: 통합 피드)
- [ ] 홈 프로모션 배너 캐러셀 (관리자에서 CRUD)
- [ ] 공지사항 (고정, 예약 게시, 푸시 알림)
- [ ] 게시글/댓글 신고
- [ ] 검색 (커뮤니티 검색 + 커뮤니티 내 게시글 검색)
- [ ] Shop 탭 (인앱 WebView → x-square.kr)
- [ ] DM 탭 플레이스홀더 ("곧 출시" + 알림받기)
- [ ] 커뮤니티 내 팔로잉/팔로워 & 커뮤니티 프로필
- [ ] Highlight 탭 (공지사항 → 캘린더 → 아티스트 게시글 → 팬 게시글 → 아티스트 프로필 모아보기)
