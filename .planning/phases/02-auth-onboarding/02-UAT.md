...
severity: major
resolution: fixed - react-i18next 이중 인스턴스 문제 해결 (f289d41)
...
result: skipped
reason: Apple Developer 설정 미완료 (별도 설정 필요)
...
### 7. DoB 생년월일 입력
expected: 네이티브 DateTimePicker 또는 웹 date input이 표시됨. 날짜 선택 후 다음으로 진행 가능. 만 14세 미만 선택 시 에러 메시지가 표시되고 진행 차단됨.
result: pass

### 8. Language 언어 선택
expected: 5개 언어(한국어, English, ไทย, 中文, 日本語) 목록이 라디오 버튼 형태로 표시됨. 언어 선택 시 앱 언어가 즉시 변경됨.
result: issue
reported: "라디오 버튼 형태로 표시는 되는데 언어 선택 시 앱 언어가 즉시 변경되지 않음"
severity: major
resolution: fixed - i18n reactivity 수정 (a1aa0df)

### 9. Creator Curation 화면
expected: 2열 그리드로 크리에이터/커뮤니티 카드가 표시됨. 멀티 선택 가능하며 탭 시 선택 애니메이션이 있음. Skip 가능.
result: skipped
reason: 테스트 데이터 없어서 멀티 선택 및 카드 디자인 확인 불가. Skip은 정상 동작.
...
result: issue
reported: "상단에 점 인디케이터 표시안됨"
severity: major
resolution: fixed - dot indicator 수정 (a1aa0df)

### 12. i18n 다국어 지원
expected: 로그인, 온보딩 전체 화면에서 선택된 언어에 맞는 텍스트가 표시됨. 언어 변경 시 즉시 반영됨.
result: issue
reported: "언어 변경 즉시반영 안됨"
severity: major
resolution: fixed - i18n reactivity 수정 (a1aa0df)
...
- truth: "언어 선택 시 앱 언어가 즉시 변경됨"
  status: resolved
  reason: "User reported: 라디오 버튼 형태로 표시는 되는데 언어 선택 시 앱 언어가 즉시 변경되지 않음"
  severity: major
  test: 8
  resolution: "fixed - i18n reactivity 수정 (a1aa0df)"

- truth: "온보딩 각 단계에서 상단에 4개의 점 인디케이터가 표시됨"
  status: resolved
  reason: "User reported: 상단에 점 인디케이터 표시안됨"
  severity: major
  test: 11
  resolution: "fixed - dot indicator 수정 (a1aa0df)"

- truth: "언어 변경 시 앱 전체 텍스트가 즉시 반영됨"
  status: resolved
  reason: "User reported: 언어 변경 즉시반영 안됨"
  severity: major
  test: 12
  resolution: "fixed - i18n reactivity 수정 (a1aa0df)"
